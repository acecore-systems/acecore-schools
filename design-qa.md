...
- Panel layout: The desktop composition keeps the AG-style one-sheet density, including the angled hero/manga boundary, 01-03 / 04-07 row structure, and the 08 final CTA strip.
...
- Overflow: All numbered desktop panels and the final CTA strip render with `scrollHeight <= clientHeight`; no hidden panel content remains.
...
- Browser/IAB could not be used because the in-app browser bridge returned `codex/sandbox-state-meta: missing field sandboxPolicy`.
