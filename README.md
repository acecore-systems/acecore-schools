...
OpenAI用1536次元indexはPreviewとProductionに作成済みですが、まだcorpusを同期していないため、
両環境とも`SEARCH_ENABLED=false`です。Preview同期とQA、Production同期、vector件数・
namespaceの照合が完了した後に、別の変更でhealthに連動した検索UIを有効化します。
